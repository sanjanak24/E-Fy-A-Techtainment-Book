...
$con = mysqli_connect($servername, $username, $password, $database);
     ...
     $id = $_SESSION['id'];
$res = mysqli_query($con, "SELECT * FROM events,tudent_register where events.fkid=tudent_register.fkid and events.ename=150.02 or events.ename=300.02 ");
...
<?php
		while($data=mysqli_fetch_array($res))
		{
			...
			<td><?php 
						  if($data["ename"]==150.02 || $data["ename"]==300.02 ){
							 echo "event3";
						 }?> </td>
			<td><?php 
