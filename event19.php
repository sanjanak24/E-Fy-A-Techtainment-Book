...
$con = mysqli_connect($servername, $username, $password, $database);
     ...
     $id = $_SESSION['id'];
$res = mysqli_query($con, "SELECT * FROM events,tudent_register where events.fkid=tudent_register.fkid and events.ename=150.03");
...
<?php
		while($data=mysqli_fetch_array($res))
		{
			...
			<td><?php 
						  if($data["ename"]==150.03 || $data["ename"]==300.03){
							 echo "event4";
						 }?> </td>
			<td><?php 
